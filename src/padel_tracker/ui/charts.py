...
def make_player_metric_history_chart(
    ...
    # Fetch data vs
    col_to_keep = [translator("date"), metric, translator("result")]
    with DB.get_session() as session:
        ...
        if metric == translator("elo_rating"):
            ...
            extra_tooltip = [translator("elo_rating_gain"), translator("result")]
    ...
    chart = base_chart.encode(
        ...
        tooltip=[x_param, y_param, color_param + ":N"] + extra_tooltip,
    ).interactive()
...
def make_player_metric_history_chart_form(
    ...
    st.markdown(
        ...
        <div style="text-align: center;">
            <div style="font-size: {font_size_header}px; font-weight: bold; margin: 0;"> Analytics </div>
            <div style="font-size: {font_size_subheader}px; margin: 0;"> {translator("evolution")} </div>
            <br>
        </div>
        """,
